...
1. **Environment (production)**
   ...
   **Server** – `server/.env`:
   ...
   - İsteğe bağlı: `RECAPTCHA_V3_SECRET` → Google reCAPTCHA v3 secret (sunucuda; kodda sabit varsayılan varsa bu satır gerekmez)
...
4. **Docker**
   ...
   **Form artık her zaman aynı origin üzerinden gider:** Tarayıcı `POST /api/contact` (Next.js) yapar; Docker’da Next.js bu isteği `USE_BACKEND_PROXY=true` ve `INTERNAL_API_URL=http://server:4000` ile Express backend’e proxy eder. Böylece `localhost`/IP sorunu kalmaz. Sunucuda `server/.env` (BREVO, vb.) tanımlı olsun; `env_file: - ./server/.env` ile yüklenir. reCAPTCHA v3 site anahtarı `client/src/lib/constants.ts` içindedir.
...
- **Form gerçekten hata veriyorsa** kontrol edin:
  ...
  - **Env**: Backend’de `BREVO_USER`, `BREVO_API_KEY` tanımlı mı? (reCAPTCHA v3 doğrulaması için isteğe bağlı `RECAPTCHA_V3_SECRET`)

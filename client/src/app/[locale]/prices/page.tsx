...
import { ChevronRight, CheckCircle, Euro, FileText, Eye, BarChart3 } from "lucide-react";
...
export function generateMetadata() {
  return {
    title: "Preise | Objekträumung",
    description: "Transparente Preisgestaltung, Festpreis mit Wertausgleich. Kostenlose Besichtigung, schriftliches Angebot. MA 48 Alternativen, Expresszuschläge.",
  };
}
...
export default async function PricesPage({ params }: Props) {
  ...
  const tUeb = await getTranslations("ueberblick");

  const pricesMetaDescription =
  ...
  const costRows = [
    { obj: tUeb("costRow1"), size: tUeb("costRow1Size"), typical: tUeb("costRow1Typical"), val: tUeb("costRow1Valuation"), final: tUeb("costRow1Final") },
    { obj: tUeb("costRow2"), size: tUeb("costRow2Size"), typical: tUeb("costRow2Typical"), val: tUeb("costRow2Valuation"), final: tUeb("costRow2Final") },
    { obj: tUeb("costRow3"), size: tUeb("costRow3Size"), typical: tUeb("costRow3Typical"), val: tUeb("costRow3Valuation"), final: tUeb("costRow3Final") },
    { obj: tUeb("costRow4"), size: tUeb("costRow4Size"), typical: tUeb("costRow4Typical"), val: tUeb("costRow4Valuation"), final: tUeb("costRow4Final") },
    { obj: tUeb("costRow5"), size: tUeb("costRow5Size"), typical: tUeb("costRow5Typical"), val: tUeb("costRow5Valuation"), final: tUeb("costRow5Final") },
  ];
    ...
    <div className="min-h-screen bg-white">
      <PageStructuredData
        graph={buildSimpleContentPageGraph({
          path: "/prices",
          name: "Preise | Objekträumung",
          description: pricesMetaDescription,
          breadcrumbs: [
            { name: tNav("home"), item: SITE_URL },
            { name: t("breadcrumb"), item: canonicalToAbsolute("/prices") },
          ],
        })}
      />
      {/* Breadcrumb + Header */}
      ...
      <article className="max-w-3xl mx-auto px-4 sm:px-6 py-10 md:py-14 space-y-12 md:space-y-16">
        ...
        <section>
            ...
            <table className="w-full min-w-[560px] text-left text-sm">
              <thead>
                <tr className="bg-navy/5 border-b border-navy/10">
                  <th className="px-4 py-3 font-semibold text-navy">{tUeb("costTableObjectType")}</th>
                  <th className="px-4 py-3 font-semibold text-navy">{tUeb("costTableSize")}</th>
                  <th className="px-4 py-3 font-semibold text-navy">{tUeb("costTableTypical")}</th>
                  <th className="px-4 py-3 font-semibold text-navy">{tUeb("costTableValuation")}</th>
                  <th className="px-4 py-3 font-semibold text-navy">{tUeb("costTableFinal")}</th>
                </tr>
              </thead>
              <tbody>
                {costRows.map((row, i) => (
          ...
          <p className="mt-4 p-4 bg-accent/10 rounded-xl text-navy/90 border border-accent/20">
            <strong className="text-accent">{tUeb("costTipLabel")}</strong> {tUeb("costTip")}
          </p>
        </section>

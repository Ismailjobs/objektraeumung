...
const ReferenzenGallery = nextDynamic(
  () => import("@/components/ReferenzenGallery").then((m) => m.ReferenzenGallery),
  { ssr: false, loading: () => <section className="min-h-[320px]" aria-hidden /> }
);
...
export function generateMetadata() {
  return {
    title: "Referenzen | Objekträumung",
    description: "So räumen wir – echte Einsätze von der Bestandsaufnahme bis zur sauberen Übergabe. Eindrücke vor Ort.",
  };
}
...
export default async function ReferenzenPage({ params }: Props) {
  ...
  const referenzenMetaDescription =
    ...
    <div className="min-h-screen bg-white">
      <PageStructuredData
        graph={buildSimpleContentPageGraph({
          path: "/referenzen",
          name: "Referenzen | Objekträumung",
          description: referenzenMetaDescription,
          breadcrumbs: [
            { name: tNav("home"), item: SITE_URL },
            { name: t("breadcrumb"), item: canonicalToAbsolute("/referenzen") },
          ],
        })}
      />

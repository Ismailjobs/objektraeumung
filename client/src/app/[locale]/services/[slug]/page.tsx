...
import { ServiceEntsorgungWienLayout } from "@/components/ServiceEntsorgungWienLayout";
...
export async function generateMetadata({ params }: Props) {
  ...
  if (!config) return buildMetadata({ title: "Objekträumung", description: "Objekträumung Wien" });
  const t = await getTranslations({ locale, namespace: "services" });
  const title = t(`${config.titleKey}.title`);
  const metaDesc = t(`${config.titleKey}.metaDescription`);
  const metadataTitle = `${title} | Objekträumung`;
  const imagePath = slug === "entsorgung-wien"
    ? "/services/entsorgung/hero.webp"
...
export default async function ServiceSlugPage({ params }: Props) {
  ...
  const isEntsorgungWien = slug === "entsorgung-wien";
  const isEinkauf = slug === "einkauf";
  const serviceUrl = canonicalToAbsolute(`/services/${slug}`);
  const faqPairs = isEntsorgungWien
    ? [
        { question: t("entsorgungWien.faq1Q"), answer: t("entsorgungWien.faq1A") },
        { question: t("entsorgungWien.faq2Q"), answer: t("entsorgungWien.faq2A") },
        { question: t("entsorgungWien.faq3Q"), answer: t("entsorgungWien.faq3A") },
        { question: t("entsorgungWien.faq4Q"), answer: t("entsorgungWien.faq4A") },
        { question: t("entsorgungWien.faq5Q"), answer: t("entsorgungWien.faq5A") },
        { question: t("entsorgungWien.faq6Q"), answer: t("entsorgungWien.faq6A") },
      ]
    : undefined;
  const useZLayout = !isEinkauf && !isHousehold && !isKeller && !isDachboden && !isVerlassenschaft && !isMessie && !isWohnungsaufloesung && !isZimmerRaeumung && !isFirmenaufloesung && !isGastroRetail && !isGaragenraeumung && !isLagerGewerbeparks && !isAntiquitaetenWertanrechnung && !isRaeumungWien && !isEntrumpelungWien && !isEntsorgungWien && !isUeberblick;
  const body1 = useZLayout ? t(`${contentKey}.body1`) : "";
  const body2 = useZLayout ? t(`${contentKey}.body2`) : "";
  const subtitle = isEinkauf ? t("einkauf.subtitle") : isHousehold ? t("household.subtitle") : isKeller ? t("kellerraeumung.subtitle") : isDachboden ? t("dachbodenraeumung.subtitle") : isVerlassenschaft ? t("verlassenschaft.subtitle") : isMessie ? t("messieEntruempelung.subtitle") : isUeberblick ? tUeb("heroSubtitle") : isWohnungsaufloesung ? t("wohnungsaufloesung.subtitle") : isZimmerRaeumung ? t("zimmerRaeumung.subtitle") : isFirmenaufloesung ? t("firmenaufloesung.subtitle") : isGastroRetail ? t("gastroRetail.subtitle") : isGaragenraeumung ? t("garagenraeumung.subtitle") : isLagerGewerbeparks ? t("lagerGewerbeparks.subtitle") : isAntiquitaetenWertanrechnung ? t("antiquitaetenWertanrechnung.subtitle") : isMoebelWertanrechnung ? t("moebelWertanrechnung.subtitle") : isRaeumungWien ? t("raeumungWien.subtitle") : isEntrumpelungWien ? t("entrumpelungWien.subtitle") : isEntsorgungWien ? t("entsorgungWien.subtitle") : undefined;

  return (
    <>
      <PageStructuredData
        graph={buildServiceSlugPageGraph({
          serviceUrl,
          headline: title,
          description: metaDesc,
          breadcrumbs: [
            { name: tNav("home"), item: SITE_URL },
            { name: tNav("services"), item: canonicalToAbsolute("/services") },
            { name: title, item: serviceUrl },
          ],
          imageUrl: serviceHeroAbsoluteUrl(slug),
          faqPairs,
        })}
      />
      ...
      <article className={theme.articleBg}>
        ...
        {isEinkauf ? (
          ...
          <ServiceEntsorgungWienLayout theme={theme} />
        ) : isUeberblick ? (

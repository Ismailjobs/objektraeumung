...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  for (const { path, priority, changeFrequency } of STATIC_PATHS) {
    ...
    const url = noLocalePrefix
      ? (pathPart ? `${base}/${pathPart}` : base)
      : (pathPart ? `${base}/${routing.defaultLocale}/${pathPart}` : `${base}/${routing.defaultLocale}`);
    entries.push({
      url,
      lastModified: now,
      changeFrequency,
  ...
  for (const { slug } of SERVICE_LIST) {
    entries.push({
      url: noLocalePrefix ? `${base}/services/${slug}` : `${base}/${routing.defaultLocale}/services/${slug}`,
      lastModified: now,

...
export function ServiceGaragenraeumungLayout({ slug, theme }: Props) {
  const t = useTranslations("services.garagenraeumung");
  ...
  const contents = [t("content1"), t("content2"), t("content3"), t("content4"), t("content5")];
  const processShort = [t("processShort1"), t("processShort2"), t("processShort3"), t("processShort4")];
  const valueItems = [t("value1"), t("value2"), t("value3"), t("value4")];
    ...
    <div className={`max-w-4xl mx-auto px-4 sm:px-6 py-10 md:py-14 ${theme.zLayoutBg}`}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid md:grid-cols-2 gap-8 md:gap-12 items-center mb-12">
        ...
        <div>
          ...
          <Link href="/#contact" className={`inline-block font-medium ${theme.accentClass} hover:underline`}>
            {t("searchCta")}
          </Link>
        </div>
      </motion.div>

      {/* Typische Garageninhalte */}
      <section className="mb-10">
        <h2 className="text-xl font-bold text-navy mb-4">{t("contentsTitle")}</h2>
      ...
      <section className="mb-10 p-4 rounded-xl border border-navy/10 bg-white">
        <h2 className="text-lg font-bold text-navy mb-4">{t("processShortTitle")}</h2>
        <ol className="space-y-2">
          {processShort.map((text, i) => (
            <li key={i} className={`flex gap-2 items-start ${theme.textColor}`}>
              <span className="shrink-0 w-6 h-6 rounded-full bg-accent/15 flex items-center justify-center text-accent text-sm font-bold">{i + 1}</span>
              {text}
            </li>
          ))}
        </ol>
      ...
      {/* Value */}
      <section className="mb-10 p-4 rounded-xl border border-navy/10 bg-navy/5">
        <h2 className="text-xl font-bold text-navy mb-2">{t("valueTitle")}</h2>
        <p className={`${theme.textColor} text-sm mb-4`}>{t("valueIntro")}</p>
        <ul className="space-y-1 text-sm mb-4">
          {valueItems.map((text, i) => (
            <li key={i} className={theme.textColor}>{text}</li>
          ))}
        </ul>
        <p className={`text-sm font-medium ${theme.accentClass}`}>{t("valueCta")}</p>
      </section>

      {/* Combo */}
      <section className="mb-10">
        <h2 className="text-lg font-bold text-navy mb-2">{t("comboTitle")}</h2>
        <p className={`${theme.textColor} text-sm mb-2`}>{t("comboDesc")}</p>
        <Link href="/services/kellerraeumung" className={`text-sm font-medium ${theme.accentClass} hover:underline`}>
          {t("comboLinkText")}
        </Link>
      </section>

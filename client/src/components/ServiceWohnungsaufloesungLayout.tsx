...
export function ServiceWohnungsaufloesungLayout({ slug, theme }: Props) {
  const t = useTranslations("services.wohnungsaufloesung");
  ...
  return (
    <div className={`max-w-4xl mx-auto px-4 sm:px-6 py-10 md:py-14 ${theme.zLayoutBg}`}>
      ...
      <motion.section initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
        ...
        <p className={`${theme.textColor} text-lg leading-relaxed mb-4`}>{t("intro2")}</p>
        <Link href="/#contact" className={`inline-block font-medium ${theme.accentClass} hover:underline`}>
          {t("introCta")}
        </Link>
      </motion.section>

      {/* Image 1 + Clear handover */}
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid md:grid-cols-2 gap-8 md:gap-12 items-center mb-12">
        ...
        <div>
          <h2 className="text-lg font-bold text-navy mb-3">{t("clearTitle")}</h2>
          <p className={`${theme.textColor} text-sm leading-relaxed`}>{t("clearDesc")}</p>
        </div>
      </motion.div>

      {/* Death case */}
      <section className="mb-10 p-4 rounded-xl border border-navy/10 bg-white">
        <h2 className="text-lg font-bold text-navy mb-2">{t("deathTitle")}</h2>
        <p className={`${theme.textColor} text-sm mb-2`}>{t("deathDesc")}</p>
        <Link href="/services/verlassenschaft" className={`text-sm font-medium ${theme.accentClass} hover:underline`}>
          {t("deathLinkText")}
        </Link>
      </section>
      ...
      <section className="mb-10">
        ...
        <p className={`${theme.textColor} text-sm mb-2`}>{t("districtsDesc")}</p>
        <Link href="/locations" className={`text-sm font-medium ${theme.accentClass} hover:underline`}>
          {t("districtsLinkText")}
        </Link>
      </section>

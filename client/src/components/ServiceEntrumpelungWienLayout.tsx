...
export function ServiceEntrumpelungWienLayout({ slug, theme }: Props) {
  const t = useTranslations("services.entrumpelungWien");
  const imageFolder = getImageFolder(slug);
  const content1 = `/services/${imageFolder}/content-1.webp`;
  const content2 = `/services/${imageFolder}/content-2.webp`;
  const [show1, setShow1] = useState(true);
  const [show2, setShow2] = useState(true);

  const bullets = [t("bullet1"), t("bullet2"), t("bullet3"), t("bullet4")];
  const whyItems = [t("why1"), t("why2"), t("why3"), t("why4")];
  const servicesItems = [t("services1"), t("services2"), t("services3"), t("services4")];
  const priceItems = [t("price1"), t("price2"), t("price3"), t("price4")];
  const processSteps = [
  ...
  const sustainabilityItems = [t("sustainability1"), t("sustainability2"), t("sustainability3"), t("sustainability4")];
  ...
  return (
    <div className={`max-w-4xl mx-auto px-4 sm:px-6 py-10 md:py-14 ${theme.zLayoutBg}`}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid md:grid-cols-2 gap-8 md:gap-12 items-center mb-12">
        {show1 ? (
          <div className={`relative aspect-[4/3] rounded-xl overflow-hidden ${theme.imageRing}`}>
            <Image src={content1} alt="" fill sizes="(max-width: 767px) 100vw, (max-width: 1152px) 50vw, 576px" quality={70} className="absolute inset-0 h-full w-full object-cover" onError={() => setShow1(false)} />
          </div>
        ) : (
          <div className="hidden md:block aspect-[4/3]" aria-hidden />
        )}
        <div>
          <h2 className="text-xl font-bold text-navy mb-3">{t("whyTitle")}</h2>
      ...
      <section className="mb-10 p-4 rounded-xl bg-navy/5 border border-navy/10">
        <h2 className="text-lg font-bold text-navy mb-2">{t("districtsTitle")}</h2>
        <p className={`${theme.textColor} text-sm`}>{t("districts")}</p>
      </section>

      {/* Leistungen */}
      <section className="mb-10">
        <h2 className="text-xl font-bold text-navy mb-3">{t("servicesTitle")}</h2>
        <ul className="space-y-2">
          {servicesItems.map((text, i) => (
            <li key={i} className={`flex gap-2 items-start ${theme.textColor}`}>
              <CheckCircle size={20} className="text-accent shrink-0 mt-0.5" /> {text}
            </li>
          ))}
      ...
      {/* Preise */}
      <section className="mb-10">
        <h2 className="text-xl font-bold text-navy mb-3">{t("pricesTitle")}</h2>
        <p className={`${theme.textColor} text-sm mb-2`}>{t("pricesIntro")}</p>
        <p className={`${theme.textColor} text-sm mb-3`}>{t("pricesRichtwerte")}</p>
        <ul className="space-y-1 mb-3">
          {priceItems.map((text, i) => (
            <li key={i} className={`${theme.textColor} text-sm`}>{text}</li>
          ))}
        </ul>
        <p className={`${theme.textColor} text-sm italic`}>{t("pricesNote")}</p>
      </section>
      ...
      <section className="mb-10">
        ...
        <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className={`inline-block mt-4 text-sm font-medium ${theme.accentClass} hover:underline`}>
          {t("processCta")}
        </a>
      </section>

      {/* Wohnungsauflösung Link */}
      <section className="mb-10 p-4 rounded-xl border border-navy/10 bg-white">
        <h2 className="text-lg font-bold text-navy mb-2">{t("wohnungsLinkTitle")}</h2>
        <p className={`${theme.textColor} text-sm mb-2`}>{t("wohnungsLink")}</p>
        <Link href="/services/wohnungsaufloesung" className={`text-sm font-medium ${theme.accentClass} hover:underline`}>
          {t("related1")}
        </Link>
        <p className={`${theme.textColor} text-sm italic mt-3`}>{t("generalLink")}</p>
      </section>

      {/* Nachhaltige Entsorgung */}
      <section className="mb-10">
        <h2 className="text-xl font-bold text-navy mb-3">{t("sustainabilityTitle")}</h2>
        <ul className="space-y-2 mb-3">
          {sustainabilityItems.map((text, i) => (
            <li key={i} className={`flex gap-2 items-start ${theme.textColor}`}>
              <CheckCircle size={20} className="text-accent shrink-0 mt-0.5" /> {text}
            </li>
          ))}
        </ul>
        <p className={`${theme.textColor} text-sm italic`}>{t("sustainabilityLink")}</p>
      </section>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="grid md:grid-cols-2 gap-8 md:gap-12 items-center mb-12">
        ...
        {show2 ? (
          <div className={`relative aspect-[4/3] rounded-xl overflow-hidden order-first md:order-last ${theme.imageRing}`}>
            <Image src={content2} alt="" fill sizes="(max-width: 767px) 100vw, (max-width: 1152px) 50vw, 576px" quality={70} className="absolute inset-0 h-full w-full object-cover" onError={() => setShow2(false)} />
          </div>
        ) : (

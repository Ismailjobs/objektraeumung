...
export function ServiceFirmenaufloesungLayout({ slug, theme }: Props) {
  const t = useTranslations("services.firmenaufloesung");
  ...
  return (
    <div className={`max-w-4xl mx-auto px-4 sm:px-6 py-10 md:py-14 ${theme.zLayoutBg}`}>
      ...
      <section className="mb-10 p-4 rounded-xl border border-navy/10 bg-white">
        <h2 className="text-xl font-bold text-navy mb-2">{t("betriebTitle")}</h2>
        <p className={`${theme.textColor} text-sm leading-relaxed`}>{t("betriebDesc")}</p>
      </section>

      {/* Büro */}
      <section className="mb-10">
        <h2 className="text-xl font-bold text-navy mb-2">{t("bueroTitle")}</h2>
        <p className={`${theme.textColor} text-sm leading-relaxed mb-4`}>{t("bueroDesc")}</p>
        <h3 className="text-lg font-semibold text-navy mb-2">{t("extrasTitle")}</h3>
